/**
 * SERP API - Video Search Operation
 *
 * Performs Google Video search and returns video results.
 */

import type {
	IExecuteFunctions,
	INodeExecutionData,
	INodeProperties,
	IDataObject,
} from 'n8n-workflow';
import { ujeebuApiGet, UJEEBU_ENDPOINTS } from '../../transport';
import { commonSerpOptions } from './common';
import type { SerpParams, SerpVideosResponse } from '../../types';

/**
 * Video search operation properties
 */
export const serpVideoSearchProperties: INodeProperties[] = [
	{
		displayName: 'Search Query',
		name: 'search',
		type: 'string',
		required: true,
		default: '',
		placeholder: 'machine learning tutorial',
		description: 'The video search query to perform',
		displayOptions: {
			show: {
				resource: ['serp'],
				operation: ['videoSearch'],
			},
		},
	},
	{
		displayName: 'Options',
		name: 'options',
		type: 'collection',
		placeholder: 'Add Option',
		default: {},
		displayOptions: {
			show: {
				resource: ['serp'],
				operation: ['videoSearch'],
			},
		},
		options: commonSerpOptions,
	},
];

/**
 * Execute video search operation
 */
export async function executeSerpVideoSearch(
	this: IExecuteFunctions,
	index: number,
): Promise<INodeExecutionData[]> {
	const search = this.getNodeParameter('search', index) as string;
	const options = this.getNodeParameter('options', index, {}) as Partial<SerpParams>;

	const params: SerpParams = {
		search,
		search_type: 'videos',
		...options,
	};

	const cleanParams = Object.fromEntries(
		Object.entries(params).filter(([, v]) => v !== undefined && v !== ''),
	);

	const response = await ujeebuApiGet(
		this,
		UJEEBU_ENDPOINTS.SERP,
		cleanParams,
	) as SerpVideosResponse;

	return [
		{
			json: response as unknown as IDataObject,
		},
	];
}

